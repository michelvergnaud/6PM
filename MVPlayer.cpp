...
MVPlayer::MVPlayer()
{
    ...
    buffer_L = new sample_t[bufferSize];
    buffer_R = new sample_t[bufferSize];

    out_L = (sample_t *) jack_port_get_buffer(output_port_left, bufferSize);
    out_R = (sample_t *) jack_port_get_buffer(output_port_right, bufferSize);
...
{
    MVNote * newNote = notes[note-FIRST_NOTE];
...
{
    if(bSustainOn)
        notes[note-FIRST_NOTE]->sustain();
    else
        notes[note-FIRST_NOTE]->stopPlay();
...
/*                case 0x78: // all sound off
                    stopAllNotes();std::cout <<"stop" << std::endl;
                    break;
...
*/
                ...
                case 0xB0: // controller
                    ...
                    par = midiEvent.data[2];
//                    if(ctrlNum == 0x01) // modulation wheel or lever
//                    {
//                        par = par *100 /127;
//                        for(int i=0;i<NB_OSC;i++)
//                            MVNote::oscData[i].vibAmp = par; // Bof
//                    }
//                    else
